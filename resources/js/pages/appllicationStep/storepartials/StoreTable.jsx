...
function StoreTable({ data, setData, isLoading, setIsLoading, setFilters, fetchStoreTable, setShowStoreForm }) {
    const { enqueueSnackbar, closeSnackbar } = useSnackbar()
    const [deltedById, setDeletedById] = useState(null)
    const handleChangePage = (event, newPage) => {
        if ((newPage + 1) > data.current_page) {
            fetchStoreTable(data.next_page_url)
        } else {
            fetchStoreTable(data.prev_page_url)
        }
    };
    ...
    const handleDeleteById = () => {
        Api.delete('/store/delete/' + deltedById)
            .then(res => {
                fetchStoreTable()
                setDeletedById(null)
            })
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-2 !text-xs flex items-center gap-2 whitespace-nowrap">
                                                <Tooltip title="Update Branch data" arrow TransitionComponent={Zoom}>
                                                    <span
                                                        onClick={() => setShowStoreForm({
                                                            mode: 'Update Store Data',
                                                            id: x.id,
                                                            store_branch_id: x?.store_branch_id,
                ...
                {deltedById && <motion.div
                    ...
                    <motion.div
                        ...
                        <nav className='flex flex-col items-center justify-center grow'>
                            ...
                            <nav className=''>Store will be removed </nav>
