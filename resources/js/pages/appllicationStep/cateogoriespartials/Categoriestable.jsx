...
const Categoriestable = ({ data, setData, isLoading, setIsLoading, setFilters, fetchCategoriesData, setShowCategoryForm }) => {
    ...
    const handleChangePage = (event, newPage) => {
        if ((newPage + 1) > data.current_page) {
            fetchCategoriesData(data.next_page_url)
        } else {
            fetchCategoriesData(data.prev_page_url)
        }
    };
    ...
    const handleCategoryDelete = () => {
        Api.post('/category/delete/' + deleteCategoryByID)
            .then(res => {
                fetchCategoriesData()
                        ...
                        <table className="w-full overflow-hidden">
                            <thead className="bg-secondary-200 ">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left rtl:text-right  whitespace-nowrap font-semibold dark:text-secondary-600">
                                        In Stock <span className='text-gray-400 text-xs'>(basic qty)</span>
                                    </th>
                                    <th className="px-6 py-3 text-left rtl:text-right  whitespace-nowrap font-semibold dark:text-secondary-600">
                                        Action
                                    </th>
                                ...
                                {data?.data && data?.data.map((x, i) => {
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-2 !text-xs whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <h6 className="mb-0 flex flex-col ">
                                                        <span className="flex gap-1 items-center">
                                                            {x.product} product {x.productsmodels} models
                                                        </span>
                                                        <span className='text-xs text-gray-400'>belongs to this category</span>
                                                    </h6>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-2 !text-xs flex items-center gap-2 whitespace-nowrap">
                                                <Tooltip title="Update category name" arrow TransitionComponent={Zoom}>
                                                    <span
                                                        onClick={() => setShowCategoryForm({
                                                            mode: 'Update Category Data',
                                                            id: x.id,
                                                            category: x.category
                                                        })}
                                                ...
                                                <Tooltip title="Remove category" arrow TransitionComponent={Zoom}>
                                                    <button onClick={() => setDeleteCategoryByID(x.id)}
                ...
                {deleteCategoryByID && <motion.div
                    ...
                    <motion.div
                        ...
                        <nav className='flex flex-col items-center justify-center grow'>
                            ...
                            <nav className=''>This action will not remove any product</nav>
                            <nav className=''>Products from this category wil be moved to others</nav>
                            <nav className='mt-2'>Category will be removed</nav>
                        ...
                        <nav className="mt-auto flex items-center w-full pb-1 px-2 gap-1 flex-col lg:flex-row">
                            ...
                            <Button onClick={() => setDeleteCategoryByID(null)} otherClasses="w-full basis-[100%] lg:basis-[50%]" neutral text="No Cancel" />
                        </nav>
                    </motion.div>
                </motion.div>}
            </AnimatePresence>
