...
function PackagingUnitTable({ data, setData, isLoading, setIsLoading, setFilters, fetchPackagingUnitTable, setShowPackagingUnitForm }) {
    const { enqueueSnackbar, closeSnackbar } = useSnackbar()
    const [deltedById, setDeletedById] = useState(null)
    const handleChangePage = (event, newPage) => {
        if ((newPage + 1) > data.current_page) {
            fetchPackagingUnitTable(data.next_page_url)
        } else {
            fetchPackagingUnitTable(data.prev_page_url)
        }
    };
    ...
    const handleDeleteById = () => {
        Api.delete('/packaging-unit/delete/' + deltedById)
            .then(res => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-2 !text-xs flex items-center gap-2 whitespace-nowrap">
                                                <Tooltip title="Update Package Selling Quantity" arrow TransitionComponent={Zoom}>
                                                    <span
                                                        onClick={() => setShowPackagingUnitForm({
                                                            mode: 'Update Packaging Quantity',
                                                            type: x.type,
                                                            id: x.id
                                                        })}
