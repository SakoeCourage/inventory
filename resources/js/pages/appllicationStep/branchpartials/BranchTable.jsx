...
function BranchTable({ data, setData, isLoading, setIsLoading, setFilters, fetchBranchedTable, setShowBranchForm }) {
    const { enqueueSnackbar, closeSnackbar } = useSnackbar()
    const [deltedById, setDeletedById] = useState(null)
    const handleChangePage = (event, newPage) => {
        if ((newPage + 1) > data.current_page) {
            fetchBranchedTable(data.next_page_url)
        } else {
            fetchBranchedTable(data.prev_page_url)
        }
    };
    ...
    const handleDeleteById = () => {
        Api.delete('/branch/delete/' + deltedById)
            .then(res => {
                fetchBranchedTable()
                setDeletedById(null)
            })
