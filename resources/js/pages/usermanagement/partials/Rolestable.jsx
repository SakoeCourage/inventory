...
const Rolestable = ({ data, setData, isLoading, setIsLoading, fetchRolesData, setShowRoleForm,setShowRolePermissionList }) => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar()
  const [deleteRoleByID, setDeleteRoleByID] = useState(null)


  const handleChangePage = (event, newPage) => {
    if ((newPage + 1) > data.current_page) {
      fetchRolesData(data.next_page_url)
    } else {
      fetchRolesData(data.prev_page_url)
    }
  };
  ...
  const handleRoleDelete = () => {
    Api.post('/user/delete/' + deleteRoleByID)
      .then(res => {
        fetchRolesData()
        setDeleteRoleByID(null)
  ...
  return (
    <div className="flex flex-col w-full min-h-[36rem] h-max relative ">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-2 !text-xs flex items-center gap-2 whitespace-nowrap">
                        ...
                        <Tooltip title="Update Role Name" arrow TransitionComponent={Zoom}>
                          <span
                        ...
                        <Tooltip title="Role Permissions" arrow TransitionComponent={Zoom}>
                          <button onClick={() => setShowRolePermissionList(x.name)}
                          ...
                          >
                            <Icon className='' icon="material-symbols:format-list-bulleted" fontSize={20} />
                          </button>
                        </Tooltip>
                      </td>
                    </tr>
      ...
      {/* pop on reset delete role */}
      <AnimatePresence>
        {deleteRoleByID && <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            <nav className="mt-auto flex items-center w-full pb-1 px-2 gap-1 flex-col lg:flex-row">
              <Button onClick={() => handleRoleDelete()} otherClasses="w-full basis-[100%] lg:basis-[50%]" info text="Yes Remove" />
              <Button onClick={() => setDeleteRoleByID(null)} otherClasses="w-full basis-[100%] lg:basis-[50%]" neutral text="No Cancel" />
            </nav>
          </motion.div>
        </motion.div>}
      </AnimatePresence>
